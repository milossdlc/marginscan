...
export const db={
 ...
 async get<T=RecordValue>(collection:string,ids:string[]):Promise<(T|null)[]>{return Promise.all(ids.map(async id=>{const row=await environment().DB.prepare('SELECT data FROM documents WHERE collection=? AND id=?').bind(collection,id).first<{data:string}>();return row?JSON.parse(row.data):null;}));},
