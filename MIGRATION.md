...
1. Configure THE_ODDS_API_KEY as an encrypted Worker secret in Cloudflare > Workers & Pages > marginscan > Settings > Variables and Secrets. AppDeploy only exposes secret names, so the existing value cannot be exported by the connector. Do not send the key in chat. Leave COLLECTOR_ENABLED=false until data is imported.
